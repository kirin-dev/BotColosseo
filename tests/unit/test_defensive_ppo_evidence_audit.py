...
def _evidence(root: Path) -> None:
    ...
    run = root / "runs/m5/defensive-ppo-main"
    ...
    smoke = root / "reports/m5/defensive/ppo-repair/smoke"
    ...
    _write(
        smoke_summary,
        {
            ...
            "checkpoint_sha256": {"defensive": candidate_hash},
            "test_cases_accessed": False,
        },
    )
    _write(
...
def test_defensive_ppo_audit_accepts_complete_hash_chain(tmp_path: Path) -> None:
    _evidence(tmp_path)

    result = audit_defensive_ppo_evidence(tmp_path)

    assert result["passed"] is True
    assert all(result["checks"].values())


def test_defensive_ppo_audit_rejects_candidate_tampering(tmp_path: Path) -> None:
    _evidence(tmp_path)
    candidate = tmp_path / "runs/m5/defensive-ppo-main/candidate-0200000.pt"
