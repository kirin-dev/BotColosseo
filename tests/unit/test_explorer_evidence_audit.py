...
def _evidence(root: Path) -> None:
    base_hash = "a" * 64
    scenario = "scenario"
    data_dir = root / "data/generated/m5/explorer"
    shards = []
    ...
    run_dir = root / "runs/m5/explorer-distillation"
    ...
    selected_checkpoint = root / "runs/m5/explorer-interpolation/alpha-050.pt"
    ...
    _write(
        root / "reports/m5/explorer/selection.json",
        {
            "passed": True,
            "grid": [0.25, 0.5, 0.75],
...
def test_explorer_evidence_audit_accepts_complete_hash_chain(tmp_path: Path) -> None:
    _evidence(tmp_path)

    result = audit_explorer_evidence(tmp_path)
...
def test_explorer_evidence_audit_reports_selected_checkpoint_tampering(
    tmp_path: Path,
) -> None:
    _evidence(tmp_path)
    checkpoint = tmp_path / "runs/m5/explorer-interpolation/alpha-050.pt"
