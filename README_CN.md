...
| M6 公开发布 | 进行中 | 四策略真实媒体与 hybrid-aware release 已验证；匿名用户评测待完成 |

当前 17.0 MB 发布归档的 SHA-256 与策略类型记录在
[release record](reports/m6/hybrid-release.json)；模型二进制不会进入普通 Git 历史。
