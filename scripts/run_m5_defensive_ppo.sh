...
CONFIG="$ROOT/configs/m5/defensive_ppo.yaml"
...
CUDA_SMOKE="$ROOT/runs/m5/defensive-ppo-cuda-smoke"
TRAIN="$ROOT/runs/m5/defensive-ppo-main"
EVAL_SMOKE="$ROOT/reports/m5/defensive/ppo-repair/smoke"
FORMAL="$ROOT/reports/m5/defensive/ppo-repair/formal"

export PYTHONPATH="$ROOT/src"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}"

if [[ -e "$TRAIN/metrics.jsonl" || -e "$FORMAL/manifest.json" ]]; then
  echo "Refusing to overwrite Defensive PPO production evidence" >&2
  exit 1
fi
...
if (
    ...
    or summary.get("style") != "defensive"
    ...
    raise SystemExit("Defensive PPO CUDA smoke identity or reward evidence is invalid")
PY
...
"$PYTHON" -u "$ROOT/scripts/evaluate_defensive.py" \
  --base-checkpoint "$BASE" \
  --defensive-checkpoint "$CANDIDATE" \
...
"$PYTHON" -u "$ROOT/scripts/audit_defensive_ppo.py" --root "$ROOT"
