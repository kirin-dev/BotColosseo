...
The page uses this centered title stack:

```text
BotColosseo
Controllable Game Bots for SFE
Search · Fight · Extract
```

The full original research phrasing may appear later in supporting text, but
`Crystal Run` is not used as the project name, page title, brand, or footer
identity.
...
1. A compact centered introduction occupies about 180–220 px:
   project name, subtitle, expanded SFE label, one-sentence description, and
   restrained GitHub and Evidence links.
...
3. At a common desktop viewport, the title stack and first video row must be
...
- Verify the exact new title stack and all four video/emote assets.
...
immediately below the compact title, shows larger emotes, preserves all evidence
claims, and passes desktop/mobile and repository verification.
