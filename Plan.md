...
上述三条 learned-policy 路线均失败后，项目作者于 2026-07-23 审核批准
product-first hybrid 路线：Defensive 与 Explorer 使用**完全相同且不更新的**
Strong Base Actor，并在 Actor logits 上增加只读取公开观测历史的确定性 governor。
公开标签必须写明 `hybrid governor`，不得描述成 learned reward-shaped policy。
旧 protective-presence、route-entropy、flank 等指标继续原样报告为诊断，不删除
也不冒充通过；产品 hard gate 改为公平观测、协议完整、retention、每对手
retention、干预边界、执行动作签名与后续匿名辨识度。

该路线的真实结果为：

- Defensive 200 局正式 validation 完整通过，Skill Retention `0.9591`，所有
  opponent retention 至少 `0.875`，最大连续干预严格为 `12`；
- Explorer Candidate A 的 200 局正式动作签名失败并保留；冻结的 B/C 小网格
  选择 C 后，Candidate C 200 局正式 validation 完整通过，Skill Retention
  `1.0026`，所有 opponent retention 至少 `0.9875`，Upper/Lower/Flank 均覆盖，
  executed-action signature distance 为 `0.0607`，高于冻结阈值 `0.05`；
...
和协议身份替代错误的跨运行 outcome 相等要求。该矩阵已于 2026-07-23 完成：
1,200/1,200、协议不一致数 0、未访问 test；四策略总体与逐对手单调性、objective
能力、风格覆盖和同档 hybrid retention 门全部通过，最低逐对手 retention 为
`0.9231`。因此 M5 产品路线完整通过。
...
当前进展：四策略 production 配置已使用通过正式门禁的 Strong Base、
