...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Select one Explorer checkpoint from the frozen alpha grid"
    )
...
def _candidate(
    ...
    identity_valid = (
        ...
        and report.get("style") == "explorer"
        and report.get("checkpoint_sha256") == checkpoint_hash
        and report.get("test_cases_accessed") is False
        and summary.get("checkpoint_sha256", {}).get("explorer") == checkpoint_hash
        and summary.get("route_entropy_estimator") == ROUTE_ENTROPY_ESTIMATOR
        and summary.get("test_cases_accessed") is False
        and manifest.get("passed") == summary.get("passed")
        and manifest.get("route_entropy_estimator") == ROUTE_ENTROPY_ESTIMATOR
    ...
    return {
        ...
        "route_entropy_delta": float(summary.get("route_entropy_delta", 0.0)),
        "route_entropy_estimator": summary.get("route_entropy_estimator"),
...
def _compare(left: dict[str, object], right: dict[str, object]) -> int:
    left_shift = float(left["route_entropy_delta"])
    right_shift = float(right["route_entropy_delta"])
    ...
    left_alpha = float(left["alpha"])
    right_alpha = float(right["alpha"])
    return -1 if left_alpha < right_alpha else int(left_alpha > right_alpha)
