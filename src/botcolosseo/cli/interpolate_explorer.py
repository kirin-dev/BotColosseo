...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Interpolate neutral and distilled Explorer checkpoints"
    )
    parser.add_argument("--neutral-checkpoint", type=Path, required=True)
    parser.add_argument("--distilled-checkpoint", type=Path, required=True)
    parser.add_argument("--alpha", type=float, choices=(0.25, 0.50, 0.75), required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = interpolate_explorer_checkpoints(
        _resolve(root, args.neutral_checkpoint),
