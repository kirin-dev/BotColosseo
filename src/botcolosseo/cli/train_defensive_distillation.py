...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Pretrain the Defensive style adapter")
    parser.add_argument(
...
def _validate_config(config: dict[str, Any]) -> None:
    if set(config) != _CONFIG_FIELDS or config.get("schema_version") != 1:
        raise ValueError("Defensive distillation config does not match schema version 1")
    ...
    if any(float(config[name]) <= 0 for name in _CONFIG_FIELDS - nonpositive_allowed):
        raise ValueError("Defensive distillation config values must be positive")
    if float(config["weight_decay"]) < 0:
        raise ValueError("Defensive distillation weight decay must be nonnegative")
    if Path(str(config["train_cases"])).name != "train.json":
        raise ValueError("Defensive distillation must use training cases")
...
def _load_base(path: Path, *, root: Path) -> tuple[AsymmetricActorCritic, str]:
    ...
    if payload.get("schema_version") != 1 or not isinstance(identity, dict):
        raise ValueError("Defensive distillation base must be an M3 checkpoint")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not isinstance(config, dict):
        raise ValueError("Defensive distillation config must be a mapping")
    _validate_config(config)
    updates = int(config["updates"]) if args.updates is None else args.updates
    if updates <= 0:
        raise ValueError("Defensive distillation updates must be positive")
    base_path = (
    ...
    if metrics_path.exists():
        raise FileExistsError(f"Defensive distillation output already exists: {metrics_path}")
    ...
    for update in range(updates):
        ...
        if not all(
            math.isfinite(value) for value in asdict(last).values() if isinstance(value, float)
        ):
            raise FloatingPointError("Defensive distillation produced non-finite metrics")
    ...
    if last is None:
        raise RuntimeError("Defensive distillation completed no updates")
    ...
    offline = evaluate_defensive_distillation(
        model, DataLoader(dataset, batch_size=int(config["batch_size"]), shuffle=False)
    )
    summary = {
