...
def audit_explorer_evidence(root: Path) -> dict[str, object]:
    root = root.expanduser().resolve()
    paths = {
        "data": root / "data/generated/m5/explorer/train-manifest.json",
        "distillation": root / "runs/m5/explorer-distillation/summary.json",
        "selection": root / "reports/m5/explorer/selection.json",
        "formal_manifest": root / "reports/m5/explorer/formal/manifest.json",
        "formal_summary": root / "reports/m5/explorer/formal/summary.json",
        "formal_episodes": root / "reports/m5/explorer/formal/episodes.jsonl",
    }
    missing = [str(path) for path in paths.values() if not path.is_file()]
    if missing:
        raise FileNotFoundError("Missing M5 Explorer evidence: " + ", ".join(missing))
    ...
    audit.add(
        ...
        "Production Explorer data gate did not pass",
    )
    ...
    audit.add(
        "data_shard_hashes",
        shard_hashes_valid,
        "Explorer data shard hash chain is invalid",
    )
    ...
    audit.add(
        ...
        and distillation.get("style") == "explorer"
        ...
        "Explorer distillation gate or its upstream identity is invalid",
    )
    audit.add(
        ...
        "Explorer distillation checkpoint hashes are invalid",
    )
    ...
    audit.add(
        ...
        and selection.get("route_entropy_estimator") == ROUTE_ENTROPY_ESTIMATOR
        and isinstance(selected, dict)
        and selected.get("eligible") is True
        and selected.get("route_entropy_estimator") == ROUTE_ENTROPY_ESTIMATOR
        ...
        "Frozen Explorer alpha selection is invalid",
    )
    ...
    audit.add(
        ...
        and formal_manifest.get("route_entropy_estimator")
        == ROUTE_ENTROPY_ESTIMATOR
        and formal_manifest.get("episodes") == 200
        and len(episode_lines) == 200
        and len(episode_keys) == 200,
        "Formal Explorer ledger is incomplete, duplicated, or hash-mismatched",
    )
    ...
    audit.add(
        ...
        and checkpoint_hashes.get("explorer") == selected_hash
        and checkpoint_hashes.get("strong_base") == base_hash,
        "Formal Explorer gate or selected checkpoint identity is invalid",
    )
    ...
    audit.add(
        ...
        "M5 Explorer scenario identity is inconsistent",
    )
    audit.add(
        "no_test_access",
        all(
        ...
        "M5 Explorer evidence does not prove zero test-case access",
    )
    return {
        "schema_version": 1,
        "stage": "m5-explorer-audit",
