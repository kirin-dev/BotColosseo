...
def audit_explorer_ppo_evidence(root: Path) -> dict[str, object]:
    root = root.expanduser().resolve()
    paths = {
        "warm_report": root / "runs/m5/explorer-interpolation/alpha-025.json",
        "warm_checkpoint": root / "runs/m5/explorer-interpolation/alpha-025.pt",
        "training": root / "runs/m5/explorer-ppo-main/summary.json",
        "candidate": root / "runs/m5/explorer-ppo-main/candidate-0200000.pt",
        "smoke_manifest": root
        / "reports/m5/explorer/ppo-repair/smoke/manifest.json",
        "smoke_summary": root
        / "reports/m5/explorer/ppo-repair/smoke/summary.json",
        "formal_manifest": root
        / "reports/m5/explorer/ppo-repair/formal/manifest.json",
        "formal_summary": root
        / "reports/m5/explorer/ppo-repair/formal/summary.json",
        "formal_episodes": root
        / "reports/m5/explorer/ppo-repair/formal/episodes.jsonl",
    }
    missing = [str(path) for path in paths.values() if not path.is_file()]
    if missing:
        raise FileNotFoundError(
            "Missing Explorer PPO evidence: " + ", ".join(missing)
        )
    ...
    audit.add(
        ...
        and training.get("style") == "explorer"
        ...
        "Explorer PPO training or candidate identity is invalid",
    )
    audit.add(
        "smoke_gate",
        ...
        and smoke_summary.get("route_entropy_estimator")
        == ROUTE_ENTROPY_ESTIMATOR
        and smoke_summary.get("checkpoint_sha256", {}).get("explorer")
        == candidate_hash
        and smoke_manifest.get("summary_sha256")
        == sha256_file(paths["smoke_summary"]),
        "Explorer PPO smoke gate or checkpoint identity is invalid",
    )
    ...
    audit.add(
        ...
        "Formal Explorer PPO ledger is incomplete or hash-mismatched",
    )
    ...
    audit.add(
        ...
        and formal_summary.get("route_entropy_estimator")
        == ROUTE_ENTROPY_ESTIMATOR
        and isinstance(gates, dict)
        and bool(gates)
        and all(value is True for value in gates.values())
        and isinstance(formal_hashes, dict)
        and formal_hashes.get("explorer") == candidate_hash
        and formal_hashes.get("strong_base")
        == training.get("base_checkpoint_sha256"),
        "Formal Explorer PPO gate or selected checkpoint identity is invalid",
    )
    ...
    audit.add(
        ...
        "Explorer PPO scenario identity is inconsistent",
    )
    audit.add(
        "no_test_access",
        all(
        ...
        "Explorer PPO evidence does not prove zero test-case access",
    )
    return {
        "schema_version": 1,
        "stage": "m5-explorer-ppo-audit",
